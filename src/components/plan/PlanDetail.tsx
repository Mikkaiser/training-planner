...
import { getCurrentBlock } from "@/lib/utils";
...
export function PlanDetail({ plan }: PlanDetailProps) {
  ...
  const currentBlockId = getCurrentBlock(plan)?.id ?? null;

  const handleAddPhase = (defaultTitle?: string) => {
    const title = window.prompt("Phase title", defaultTitle ?? `Phase ${phases.length + 1}`);
    if (!title) return;

    const nextOrderIndex = phases.reduce((max, phase) => Math.max(max, phase.order_index), 0) + 1;
